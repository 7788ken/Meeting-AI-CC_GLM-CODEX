...
import { transcriptEventSegmentationApi, type TranscriptEventSegment } from '@/services/api'
import {
  websocket,
...
export const useTranscriptEventSegmentationStore = defineStore(
  'transcriptEventSegmentation',
  () => {
    ...
    const pointerEventIndex = computed(
      () => progress.value?.pointerEventIndex ?? latestProcessedEventIndex.value
    )
