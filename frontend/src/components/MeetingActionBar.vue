  ...
  <div class="actionbar-host" :aria-hidden="!enabled">
    <div v-if="enabled" class="actionbar app-surface">
      <div class="left">
        <div class="hint">
          ...
          <span class="hint-item"><kbd>P</kbd>提示词</span>
          <span class="hint-item"><kbd>?</kbd>帮助</span>
        </div>
      </div>

      <div class="right">
        <el-button size="small" :disabled="disabled" @click="emit('toggle-recording')">
          {{ recordButtonText }}
        </el-button>
        <el-button
          size="small"
        ...
        <el-button size="small" type="danger" :disabled="disabled || ending" @click="emit('end-session')">
          结束会话
        </el-button>

        <template v-if="!compact">
          <el-button size="small" :disabled="disabled" @click="emit('open-prompts')">提示词</el-button>
          <el-button size="small" :disabled="disabled" @click="emit('toggle-realtime')">原文流</el-button>
          <el-button size="small" class="ghost" @click="helpVisible = true">快捷键</el-button>
        </template>
        ...
        <template v-else>
          ...
          >
            <el-button size="small" class="ghost" :class="[moreVisible ? 'is-active' : '']">更多</el-button>
            <template #dropdown>
              <el-dropdown-menu>
                <el-dropdown-item command="prompts">提示词</el-dropdown-item>
                <el-dropdown-item command="toggleRealtime">原文流折叠/展开</el-dropdown-item>
    ...
    <el-dialog v-model="helpVisible" title="快捷键" width="min(92vw, 560px)" append-to-body>
      <div class="help-grid">
        ...
        <div class="help-row">
          <kbd>P</kbd>
          <div class="help-text">选择提示词模板</div>
        </div>
        <div class="help-row">
          <kbd>?</kbd>
...
const canToggleMute = computed(() => props.recordingStatus === 'recording' || props.recordingStatus === 'paused')
