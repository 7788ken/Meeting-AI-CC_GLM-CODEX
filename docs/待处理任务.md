...
**已处理（WS onclose/onerror 即时 stopTranscribe + 状态回滚；断线提示与重试入口已补）。**
2. 拆分窗口未命中 previousSentence 时需自动扩窗或触发重建，避免拆分停滞。
**已处理（扩窗 + rebuildFromStart）。**
3. `GlmRateLimiter` 全局 + 桶双层排队导致有效并发下降，需评估调度策略。
**已处理（补充排队耗时 P50/P95 统计，用于量化调度影响并评估吞吐）。**
4. 429 的 sleep 与 `GlmRateLimiter.onRateLimit` 双重退避叠加，需统一退避策略。
**已处理（统一由 RateLimiter 冷却接管，无额外 sleep 退避）。**
5. 分段调度并发控制与桶并发重复，等待任务占用 inFlight 导致吞吐下降。
**已处理（新增 `TRANSCRIPT_EVENTS_SEGMENT_MAX_IN_FLIGHT` 限制并发会话数）。**
...
~~已处理~~
10. 语句拆分进行中的时候，如果用户当前的焦点在列表中的某一个记录，则中语句拆分列表更新的时候不自动滚动列表，保持现状，因为当列表更新到时候自动滚动到最新记录会破坏用户的浏览体验。如果用户滚动到最后一条记录则恢复滚动到最新记录。
~~已处理~~
...
~~已处理~~
2. 当分析卡住的时候（发生各种意外而无法返回流式数据），不能只显示“点击语句卡片上的"针对性分析"按钮生成分析“，应该还有一个重试按钮。
~~已处理~~
3. 增加 LLM 接口，可以切换到 DeepSeek LLM 模型。设置页面要增加 DeepSeek LLM 模型的配置项（api-url, key）。
**未处理**


## 全文分析总结
1. 添加一个内置提示词 2，用于分析本次面试的内容结果，打分，并分析其中不足的地方，复盘，并分析面试官的想法，以及其他方面。
**未处理**
...
4. 日志记录设置：增加开关记录日志按钮：LLM的请求与回复/错误/系统日志，并把对应日志落库到 app_logs 表中，并在会话调试台中增加相应的 tab 切换显示，备注只有在打开时候才记录。
~~已处理~~
...
7. 设置已保存，但服务配置同步失败.websocket.ts:198 [WebSocket] 连接成功
~~已处理~~ 

## 中控大屏 
### 前端
1. 并发请求运行中控大屏（`/ops/concurrency-dashboard`），当没有授权的时候应该提示用户授权，并且刷新不需要重新授权，一次授权有效 1 天。
~~已处理~~ 
2. 运行中会话,含义应该为录音中或者30 秒内有请求的会话。
**未处理**
...
~~已处理~~
2. 友好提示websocket.ts:198 [WebSocket] 连接成功
transcription.ts:305 [Transcription] 状态消息 {sessionId: '37c392b3-cecd-4209-9108-a1db216e989a', status: 'session_set'}
websocket.ts:224 [WebSocket] 连接关闭
HomeView.vue?t=1768816030019:252 [Vue warn]: Property "openConcurrencyDashboard" was accessed during render but is not defined on instance. 
  at <HomeView onVnodeUnmounted=fn<onVnodeUnmounted> ref=Ref< Proxy(Object) {__v_skip: true} > > 
  at <RouterView> 
  at <App>
warn$1 @ runtime-core.esm-bundler.js:51
get @ runtime-core.esm-bundler.js:3371
_sfc_render @ HomeView.vue?t=1768816030019:252
renderComponentRoot @ runtime-core.esm-bundler.js:4497
componentUpdateFn @ runtime-core.esm-bundler.js:6122
run @ reactivity.esm-bundler.js:237
(匿名) @ runtime-core.esm-bundler.js:499
rerender @ runtime-core.esm-bundler.js:491
(匿名) @ runtime-core.esm-bundler.js:565
(匿名) @ HomeView.vue:580
(匿名) @ client:66
(匿名) @ client:218
(匿名) @ client:196
...
(匿名) @ client:413
websocket.ts:198 [WebSocket] 连接成功
MeetingView.vue:1259 开始录音失败: Error: 音频采集权限被拒绝（麦克风/共享音频）
    at AudioCaptureService.normalizeGetUserMediaError (audioCapture.ts:162:46)
    at AudioCaptureService.requestPermission (audioCapture.ts:82:31)
    at async AudioCaptureService.startCapture (audioCapture.ts:188:27)
    at async TranscriptionService.start (transcription.ts:152:7)
    at async startRecording (MeetingView.vue:1233:5)
    at async toggleRecording (MeetingView.vue:1207:5)
startRecording @ MeetingView.vue:1259
await in startRecording
toggleRecording @ MeetingView.vue:1207
callWithErrorHandling @ runtime-core.esm-bundler.js:199
callWithAsyncErrorHandling @ runtime-core.esm-bundler.js:206
emit @ runtime-core.esm-bundler.js:4380
(匿名) @ runtime-core.esm-bundler.js:8232
$props.enabled._createVNode.onClick._cache.<computed>._cache.<computed> @ MeetingActionBar.vue:27
callWithErrorHandling @ runtime-core.esm-bundler.js:199
callWithAsyncErrorHandling @ runtime-core.esm-bundler.js:206
emit @ runtime-core.esm-bundler.js:4380
(匿名) @ runtime-core.esm-bundler.js:8232
